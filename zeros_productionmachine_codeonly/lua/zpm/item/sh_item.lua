...
function zpm.Item.GetRecipeList(ply)
	...
	for k,v in ipairs(zpm.config.Items) do
		...
		if not zpm.Item.CanMake(k,ply) then continue end
		table.insert(list,k)
	end
	return list
end
...
/*
	Returns if the player can make this recipe
*/
function zpm.Item.CanMake(itemid,ply)
	...
	if not dat.machine then return false end

	//if dat.jobs and istable(ProductData.jobs) and table.Count(ProductData.jobs) > 0 and not dat.jobs[ply:Team()] then return false end
